...
// Função para processar EAN do coletor removendo primeiro e último dígito
function processarEanColetor(eanBipado) {
  ...
  // Se o EAN tem pelo menos 3 dígitos, remove o primeiro e último
  if (eanLimpo.length >= 3) {
    const eanProcessado = eanLimpo.substring(1, eanLimpo.length - 1);
    console.log(`[EAN] Original: ${eanLimpo} -> Processado: ${eanProcessado}`);
    return eanProcessado;
  }
  
  // Se muito curto, retorna o original
  console.log(`[EAN] EAN muito curto, mantendo original: ${eanLimpo}`);
  return eanLimpo;
}

// Função inteligente para encontrar item por EAN
function encontrarItemPorEan(eanBipado) {
  if (!ctrcObj || !ctrcObj.itens) {
    return null;
  }
  
  console.log(`[EAN] === BUSCA INTELIGENTE INICIADA ===`);
  console.log(`[EAN] 📱 EAN Bipado: "${eanBipado}"`);
  
  // PRIORIDADE 1: Busca EXATA primeiro (sem processar nada)
  console.log(`[EAN] 🔍 Tentativa 1: Busca exata sem processamento`);
  let item = ctrcObj.itens.find(i => i.ean === eanBipado);
  if (item) {
    console.log(`[EAN] ✅ SUCESSO! Encontrado com EAN exato (sem processamento)`);
    console.log(`[EAN] 📦 Produto: ${item.produto}`);
    console.log(`[EAN] 🏷️ EAN: ${item.ean}`);
    return item;
  }
  
  // PRIORIDADE 2: Se não encontrou exato, verifica se o bipado parece ter dígitos extras
  // (se tem mais de 13 dígitos ou começa/termina com padrões específicos)
  const podeSerProcessado = eanBipado.length > 13 || 
                            /^[0-9][0-9]{12,}[0-9]$/.test(eanBipado);
  
  if (podeSerProcessado) {
    console.log(`[EAN] 🤔 EAN parece ter dígitos extras (${eanBipado.length} dígitos)`);
    console.log(`[EAN] 🔍 Tentativa 2: Removendo primeiro e último dígito`);
    
    const eanProcessado = processarEanColetor(eanBipado);
    console.log(`[EAN] ⚙️ EAN Processado: "${eanProcessado}"`);
    
    // Tenta encontrar o EAN processado na lista
    item = ctrcObj.itens.find(i => i.ean === eanProcessado);
    if (item) {
      console.log(`[EAN] ✅ SUCESSO! EAN processado encontrou match exato na lista`);
      console.log(`[EAN] 📦 Produto: ${item.produto}`);
      console.log(`[EAN] 🏷️ EAN Lista: ${item.ean}`);
      console.log(`[EAN] 🔄 Transformação: "${eanBipado}" → "${eanProcessado}"`);
      return item;
    }
  }
  
  // PRIORIDADE 3: Verifica se o EAN bipado pode ser encontrado processando os EANs da lista
  console.log(`[EAN] 🔍 Tentativa 3: Processando EANs da lista para comparar`);
  item = ctrcObj.itens.find(i => {
    // Se o EAN da lista tem dígitos extras, processa ele
    if (i.ean.length > 13) {
      const eanListaProcessado = processarEanColetor(i.ean);
      const match = eanBipado === eanListaProcessado;
      if (match) {
        console.log(`[EAN] 🔄 EAN da lista "${i.ean}" processado: "${eanListaProcessado}"`);
      }
      return match;
    }
    return false;
  ...
  if (item) {
    console.log(`[EAN] ✅ SUCESSO! EAN bipado encontrou EAN processado da lista`);
    console.log(`[EAN] 📦 Produto: ${item.produto}`);
    console.log(`[EAN] 🏷️ EAN Lista Original: ${item.ean}`);
    console.log(`[EAN] 🔄 EAN Lista Processado: ${processarEanColetor(item.ean)}`);
    return item;
  }
  
  // PRIORIDADE 4: Se ambos podem ter dígitos extras, compara ambos processados
  if (podeSerProcessado) {
    console.log(`[EAN] 🔍 Tentativa 4: Comparando ambos processados`);
    const eanBipadoProcessado = processarEanColetor(eanBipado);
    
    item = ctrcObj.itens.find(i => {
      if (i.ean.length > 13) {
        const eanListaProcessado = processarEanColetor(i.ean);
        const match = eanBipadoProcessado === eanListaProcessado;
        if (match) {
          console.log(`[EAN] 🔄 Bipado "${eanBipado}" → "${eanBipadoProcessado}"`);
          console.log(`[EAN] 🔄 Lista "${i.ean}" → "${eanListaProcessado}"`);
        }
        return match;
      }
      return false;
    ...
    if (item) {
      console.log(`[EAN] ✅ SUCESSO! Ambos processados são iguais`);
      console.log(`[EAN] 📦 Produto: ${item.produto}`);
      console.log(`[EAN] 🏷️ EAN Lista: ${item.ean}`);
      return item;
    }
  ...
  // PRIORIDADE 5: Busca por similaridade (últimos dígitos - fallback)
  console.log(`[EAN] 🔍 Tentativa 5: Busca por similaridade (fallback)`);
  const eanParaComparar = podeSerProcessado ? processarEanColetor(eanBipado) : eanBipado;
  const ultimosDigitos = eanParaComparar.slice(-8);
  
  item = ctrcObj.itens.find(i => {
    const eanLista = i.ean.length > 13 ? processarEanColetor(i.ean) : i.ean;
    return eanLista.includes(ultimosDigitos) || ultimosDigitos.includes(eanLista.slice(-6));
  });
  
  if (item) {
    console.log(`[EAN] ⚠️ ENCONTRADO por similaridade (pode não ser exato)`);
    console.log(`[EAN] 📦 Produto: ${item.produto}`);
    console.log(`[EAN] 🏷️ EAN Lista: ${item.ean}`);
    console.log(`[EAN] 🔍 Critério: Últimos dígitos "${ultimosDigitos}"`);
    return item;
  }
  
  // Não encontrou
  console.log(`[EAN] ❌ NÃO ENCONTRADO em nenhuma tentativa`);
  console.log(`[EAN] 📋 EANs disponíveis na lista:`);
  ctrcObj.itens.forEach((item, index) => {
    const temDigitosExtras = item.ean.length > 13;
    const eanProcessadoLista = temDigitosExtras ? processarEanColetor(item.ean) : item.ean;
    console.log(`[EAN]   ${index + 1}. "${item.ean}"${temDigitosExtras ? ` → "${eanProcessadoLista}"` : ''} | ${item.produto}`);
  });
  console.log(`[EAN] === BUSCA FINALIZADA ===`);
  
  return null;
}

// Função de teste para verificar processamento de EAN (use no console do navegador)
function testarEan(eanOriginal) {
  console.log('');
  console.log('🧪 === TESTE COMPLETO DE EAN ===');
  console.log(`📱 EAN Bipado: "${eanOriginal}"`);
  console.log(`⚙️ EAN Processado: "${processarEanColetor(eanOriginal)}"`);
  
  if (ctrcObj && ctrcObj.itens) {
    console.log('');
    console.log('🔍 Iniciando busca...');
    const item = encontrarItemPorEan(eanOriginal);
    
    console.log('');
    if (item) {
      console.log(`🎉 SUCESSO! Item encontrado:`);
      console.log(`   📦 Produto: ${item.produto}`);
      console.log(`   🏷️ EAN Lista: ${item.ean}`);
      console.log(`   📊 Quantidade: ${item.quantidade}`);
      console.log(`   ✅ Status: ${item.status || 'Pendente'}`);
    } else {
      console.log('❌ FALHOU! Item não encontrado');
    }
  } else {
    console.log('⚠️ Nenhum CTRC carregado para teste');
  }
...
function testarCenarios() {
  console.log('🎯 === TESTE DE TODOS OS CENÁRIOS ===');
  
  if (!ctrcObj || !ctrcObj.itens) {
    console.log('⚠️ Carregue um CTRC primeiro!');
    return;
  }
  
  const eansTeste = [
    '7891350034950',    // EAN normal
    '17891350034957',   // EAN com dígitos extras
    '789135003495',     // EAN processado
    '1234567890123',    // EAN inexistente
  ];
  
  eansTeste.forEach((ean, index) => {
    console.log(`\n--- Teste ${index + 1}: ${ean} ---`);
    testarEan(ean);
  });
  ...
  console.log('[DEBUG] HTML inserido. resultadoDiv childNodes:', resultadoDiv.childNodes.length);
      ...
      if (bipInputGlobal) {
        ...
        async function processBip(valorBipado) {
          ...
          const item = encontrarItemPorEan(valorBipado);
          if (!item) {
            mostrarBipagemErro(`EAN não encontrado na lista! Bipado: ${valorBipado} (Processado: ${processarEanColetor(valorBipado)})`);
            bipInputGlobal.value = '';
            ensureBipInputFocus();
            return;
          }
