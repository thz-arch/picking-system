...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const telaChecklist = document.getElementById('telaChecklist');
  ...
  function mostrarTelaHistorico() {
    ...
    telaChecklist.style.display = 'none';
  ...
  function mostrarTelaChecklist(ctrc) {
    telaFiliais.style.display = 'none';
    telaCtrcList.style.display = 'none';
    telaSeparacao.style.display = 'none';
    telaHistorico.style.display = 'none';
    telaChecklist.style.display = 'block';
    sessionStorage.setItem('tela_atual', 'checklist');
    
    const iframe = document.getElementById('iframeChecklist');
    if (iframe) {
      iframe.src = `/checklist/?ctrc=${ctrc}&mode=embedded`;
    }
  ...
  btnFinalizarPicking.addEventListener('click', async () => {
    ...
    try {
      ...
      const ctrc = resultado.picking.ctrc;
      
      // Oferece para gerar checklist
      setTimeout(async () => {
        const gerarChecklist = await UIComponents.showConfirm(
          'Deseja gerar o checklist para este CTRC agora?',
          'Picking Finalizado'
        );
        
        if (gerarChecklist) {
          mostrarTelaChecklist(ctrc);
        } else {
          mostrarTelaCtrcList();
          listarCtrcs();
        }
